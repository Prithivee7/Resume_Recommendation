...
dict_da_1 = dict(sorted(df1.items(), key=lambda x: x[1], reverse=True)[6:22])
...
percent = [str(int((i/2900) * 100))+"%" for i in x_list]
...
fig3.update_layout(
    title={
        'text': "Extracted from 2900 Job Requirements",
        'y': 0.9,
        'x': 0.5,
        'xanchor': 'center',
        'yanchor': 'top'},
...
def figures_to_html(figs, filename="data_analyst3.html"):
    ...
    for fig in figs:
        dashboard2.write(
            '<h2 style="text-align: center">Top 10 skills for data analyst for experience of 5-8 years in 2021 April</h2>')
