...
dict_da_1 = dict(sorted(df1.items(), key=lambda x: x[1], reverse=True)[5:16])
...
y_list = ['DL' if i == 'Dl' else i for i in y_list]
y_list = ['Model building' if i == 'Model' else i for i in y_list]
y_list = ['SQL' if i == 'Sql' else i for i in y_list]
y_list = ['Statistics' if i == 'Stat' else i for i in y_list]
percent = [str(int((i/3406) * 100))+"%" for i in x_list]
...
fig1.update_layout(
    title={
        'text': "Extracted from 3406 Job Requirements",
        'y': 0.9,
        'x': 0.5,
        'xanchor': 'center',
        'yanchor': 'top'},
...
def figures_to_html(figs, filename="data_science1.html"):
    dashboard2 = open(filename, 'w')
    dashboard2.write("<html><head></head><body>" + "\n")
    for fig in figs:
        dashboard2.write(
            '<h2 style="text-align: center">Top 10 skills for data scientist for experience of 0-2 years in 2021</h2>')
